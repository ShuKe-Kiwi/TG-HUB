...
def test_p6_2c_0_external_watchlist_source_channels_are_prechecked() -> None:
    ...
    assert len(results) == len(watchlist.enabled_source_refs())
    assert all(result.input_type != "disabled" for result in results)
    assert all(
        result.status
        in {"parsed_numeric_id", "username_requires_resolution", "invalid_ref"}
    ...
    assert all(ref not in serialized for ref in watchlist.enabled_source_refs())
